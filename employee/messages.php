...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action']??'') === 'send') {
    ...
    @file_get_contents($webhook, false, stream_context_create([
        'http' => ['method'=>'POST', 'header'=>"Content-Type: application/json\r\n", 'timeout'=>3,
            'content'=>json_encode([
                'event' => 'new_message', 'from' => 'employee', 'from_name' => $user['name'],
                'from_id' => $eid, 'to_type' => 'admin', 'message' => $_POST['message'],
                'job_id' => $_POST['job_id'] ?? null,
            ])]
    ]));

    header("Location: /employee/messages.php?sent=1"); exit;
}
  ...
  <form method="POST" class="space-y-3">
    ...
    <div>
      ...
      <select name="job_id" class="w-full px-3 py-2.5 border rounded-xl">
        ...
        <option value="<?= $j['j_id'] ?>">Job #<?= $j['j_id'] ?> — <?= date('d.m', strtotime($j['j_date'])) ?> <?= e($j['stitle']) ?> (<?= e($j['cname']) ?>)</option>
        <?php endforeach; ?>
      </select>
    </div>
    <div>
      <textarea name="message" required rows="3" placeholder="Nachricht eingeben..." class="w-full px-3 py-2.5 border rounded-xl"></textarea>
    </div>
...
<div class="bg-white rounded-xl border">
  ...
  <div class="divide-y">
    <?php foreach ($messages as $m):
      $isMine = $m['sender_type'] === 'employee' && (int)$m['sender_id'] === $eid;
    ?>
