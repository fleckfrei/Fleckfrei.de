...
try {
    ...
    // Settings (one DB roundtrip)
    $s = one("SELECT discount_active, discount_weekly, discount_biweekly, discount_monthly,
                     competitive_mode, competitive_premium_pct, new_customer_discount_pct,
    ...
    $freqDisc = !empty($s['discount_active']) ? [
        'woechentlich' => (float)($s['discount_weekly'] ?? 7) / 100,
        '2wochen'      => (float)($s['discount_biweekly'] ?? 5) / 100,
        'monatlich'    => (float)($s['discount_monthly'] ?? 3) / 100,
        'einmalig'     => 0,
    ...
    $compMode = (int)($s['competitive_mode'] ?? 0);
    $compPremium = (float)($s['competitive_premium_pct'] ?? 5);
    $newCustDisc = (float)($s['new_customer_discount_pct'] ?? 0);

    // Min-Price per customer-type from tiers
    $minPrices = [];
    foreach ($tiers as $t) {
    ...
    // Admin-Overrides (priority over auto-calc)
    if (!empty($s['website_price_private_override'])) $minPrices['private'] = (float)$s['website_price_private_override'];
    if (!empty($s['website_price_str_override']))     $minPrices['str']     = (float)$s['website_price_str_override'];
    if (!empty($s['website_price_office_override']))  $minPrices['office']  = (float)$s['website_price_office_override'];

    $titles = [
        'home_care' => $s['website_title_home_care'] ?? 'Home Care',
        'str'       => $s['website_title_str']       ?? 'Short-Term Rental',
        'office'    => $s['website_title_office']    ?? 'Business & Office',
    ];
