...
if ($_SERVER['REQUEST_METHOD']==='POST' && !empty($_POST['scan_id'])) {
    ...
    if ($cust) {
        $r = ['customer'=>$cust['name'], 'email'=>$cust['email'], 'time'=>date('H:i:s'), 'checks'=>[]];
        $email = $cust['email'];
        if ($email) {
            $domain = substr($email, strpos($email,'@')+1);
            $mx = @dns_get_record($domain, DNS_MX);
            $free = in_array($domain, ['gmail.com','yahoo.com','hotmail.com','outlook.com','gmx.de','web.de','t-online.de','aol.com','icloud.com','protonmail.com']);
        ...
        if (!empty($web)) {
            ...
            $r['checks']['http'] = $code;
            if ($html) { preg_match('/<title[^>]*>([^<]+)</',$html,$m); $r['checks']['title']=$m[1]??''; $t=[]; if(stripos($html,'wp-content')!==false)$t[]='WordPress'; if(stripos($html,'shopify')!==false)$t[]='Shopify'; if(stripos($html,'bootstrap')!==false)$t[]='Bootstrap'; if(stripos($html,'gtag')!==false)$t[]='Analytics'; $r['checks']['tech']=$t; }
        }
        $scan_result = $r;
    }
}
...
<div class="bg-white rounded-xl border border-brand/30 mb-6">
  ...
  <div class="p-5 grid grid-cols-2 md:grid-cols-4 gap-4">
    ...
    <?php endif; ?>
    <div class="bg-gray-50 rounded-lg p-3"><h4 class="text-xs font-semibold text-gray-400 mb-2">HTTP</h4><p class="text-2xl font-bold <?= ($s['checks']['http']??0)>=200&&($s['checks']['http']??0)<400?'text-green-600':'text-red-600' ?>"><?= $s['checks']['http']??'?' ?></p>
