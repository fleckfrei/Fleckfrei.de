...
<?php if(!empty($_GET['converted']) && !empty($_GET['token'])):
  $_plink = 'https://app.' . SITE_DOMAIN . '/p/' . htmlspecialchars($_GET['token']);
?>
<div class="bg-gradient-to-r from-green-50 to-brand-light border-2 border-green-400 text-green-900 px-5 py-4 rounded-xl mb-4">
  <div class="font-bold mb-1">✨ Lead erfolgreich umgewandelt — Kunde angelegt</div>
  <div class="text-sm mb-2">Persönlicher Buchungs-Link (30 Tage gültig, direkt an Kunden senden):</div>
  <div class="flex gap-2 items-center">
    <input type="text" readonly value="<?= $_plink ?>" onclick="this.select()" class="flex-1 px-3 py-2 bg-white border rounded-lg font-mono text-xs"/>
    <button type="button" onclick="navigator.clipboard.writeText('<?= $_plink ?>').then(()=>{this.textContent='✓ Kopiert'})" class="px-3 py-2 bg-brand text-white rounded-lg text-sm font-semibold">📋 Kopieren</button>
    <a href="/admin/prebook-links.php" class="px-3 py-2 bg-white border rounded-lg text-sm font-semibold">→ Alle Links</a>
  </div>
</div>
<?php endif; ?>
